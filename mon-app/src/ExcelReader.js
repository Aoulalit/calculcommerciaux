...
export default function ExcelTarifCalculator() {
  ...
  const [distanceSource, setDistanceSource] = useState("manuel");
  ...
  const [forfaitKm, setForfaitKm] = useState(0);
  const [forfaitDeliveries, setForfaitDeliveries] = useState(1);

  const [invoiceStyle, setInvoiceStyle] = useState("transitainer");

  const [loadingRoute, setLoadingRoute] = useState(false);
  const [routeErr, setRouteErr] = useState("");

  const [customFields, setCustomFields] = useState([
    { id: 1, label: "Référence devis", value: "" },
  ]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const match = rows.find((r) =>
        norm(r["lieu (ville/secteur)"]).includes(norm(B.label))
      );
  ...
  const styles = {
    ...
    btn: {
      padding: "10px 14px",
      borderRadius: 12,
      border: "1px solid rgba(137, 165, 255, .45)",
      background: "linear-gradient(180deg, #233067, #1b2551)",
      color: "var(--text)",
      fontWeight: 700,
      cursor: "pointer",
      textDecoration: "none",
    ...
    btnDanger: {
      padding: "10px 14px",
      borderRadius: 12,
      border: "1px solid rgba(255, 123, 151, .5)",
      background: "linear-gradient(180deg, #3a1120, #2a0d18)",
      color: "#ffdbe3",
      cursor: "pointer",
    },
    invoiceWrap: {
  ...
  async function handleExportPDF() {
    ...
    pdf.save(`Facture_Lapeyre_${todayISO()}.pdf`);
  }

  /* ========== Export PDF (Transitainer) ========== */
  function handleExportPDFTransitainer() {
    ...
    const lignes = [
      {
        date: dateFacture,
        lib: `Main d'œuvre (durée ${eur(res.dureeH)} h)`,
        qte: 1,
        unit: "U",
        pu: "",
        tva: `${tauxTVA}%`,
      ...
      },
      {
        date: dateFacture,
        lib: useForfait
          ? `Déplacement (forfait ${eur(forfaitKm)} km, ${Math.max(1, forfaitDeliveries)} livr.)`
          : "Déplacement (forfait + km Excel)",
        qte: 1,
        unit: "U",
        pu: "",
        tva: `${tauxTVA}%`,
    ...
    if ((res.mRemise || 0) > 0) {
      lignes.push({
        date: dateFacture,
        lib: `Remise (${eur(res.remiseAppliquee)} %)`,
        qte: 1,
        unit: "U",
        pu: "",
        tva: `${tauxTVA}%`,
        mht: -Math.abs(res.mRemise),
      });
    }
    ...
    pdf.setFontSize(10);
    y += 5;
    pdf.text("SAS — SIRET : à compléter", left(15), y);
    y += 5;
    pdf.text("TVA intracom : à compléter", left(15), y);
    y += 5;
    pdf.text("Adresse siège : à compléter", left(15), y);
    y += 5;
    pdf.text("contact@lapeyre-logistique.fr • +33 1 23 45 67 89", left(15), y);
    ...
    pdf.text(`Date : ${dateFacture.split("-").reverse().join("/")}`, right(15), topRight + 12, {
      align: "right",
    });

    y = 45;
    pdf.setFont(undefined, "bold");
    pdf.text("Facturer à", left(15), y);
    pdf.setFont(undefined, "normal");
    y += 5;
    pdf.text(client, left(15), y);

    clientAdr.split("\n").forEach((line) => {
      y += 5;
      pdf.text(line, left(15), y);
    });

    const trajet = useParis
      ? `${(PARIS_ARR.find((a) => a.id === arrDepart) || {}).label || "—"} → ${(PARIS_ARR.find(
          (a) => a.id === arrArrivee
        ) || {}).label || "—"}`
      : "Distance manuelle";

    const infoTrajet = `Trajet: ${trajet}  •  ${eur(distance)} km  •  ${minutes} min  •  ${
      useForfait ? "Forfait livraison" : "Tarif Excel"
    }`;
    ...
    pdf.setFontSize(9);
    pdf.setFont(undefined, "normal");
    pdf.text("TVA acquittée sur les encaissements", 15, yFoot);
    yFoot += 5;
    pdf.text("Mode de règlement : Virement", 15, yFoot);
    yFoot += 5;
    pdf.text("RIB : À compléter (Banque / IBAN / BIC)", 15, yFoot);
    yFoot += 5;
  ...
  return (
    <div className="edi-shell">
      <div className="edi-header">
        <div className="edi-brand">
          <div className="edi-logo">E</div>
          <div>
            <div className="edi-title">EDI Flow – Tarification</div>
            <div className="edi-subtitle">
              Module les tarifs, calcule les prestations et génère une facture PDF.
            </div>
          </div>
        </div>
      </div>
            ...
            <>
              <div style={styles.row}>
                ...
                <select
                  style={styles.select}
                  value={arrDepart}
                  onChange={(e) => setArrDepart(Number(e.target.value))}
                >
                  {PARIS_ARR.map((a) => (
              ...
              <div style={styles.row}>
                ...
                <select
                  style={styles.select}
                  value={arrArrivee}
                  onChange={(e) => setArrArrivee(Number(e.target.value))}
                >
                  {PARIS_ARR.map((a) => (
                    <option key={a.id} value={a.id}>
                      {a.label}
              ...
              {loadingRoute ? (
                ...
                <div style={{ ...styles.small, color: "#ff7b97" }}>{routeErr}</div>
              ) : (
                <div style={{ marginTop: 6 }}>
                  <strong>Distance (vélo)</strong> :{" "}
                  <span style={{ fontWeight: 700 }}>{fmt(distance, 2)} km</span> •{" "}
                  <strong>Temps (vélo)</strong> :{" "}
                  ...
                  <div style={styles.btnRow}>
                    <a
                      ...
                      href={`https://fr.mappy.com/itineraire#/velo/${encodeURIComponent(
                        ...
                        PARIS_ARR.find((a) => a.id === arrArrivee)?.label || ""
                      )}/1`}
                      target="_blank"
                      rel="noreferrer"
                    >
        ...
        <div style={styles.card}>
          ...
          {useForfait && (
            <>
              ...
              <div style={styles.small}>
                (Règle : 37 € si ≤ 3 km, sinon +3,10 € par tranche de 3 km entamée)
              </div>
              ...
              <div style={styles.small}>
                Chaque livraison supplémentaire au-delà de la 1ʳᵉ ajoute 10 €.
              </div>
          ...
          <div style={styles.grid2}>
            <div>
              ...
              <div style={styles.row}>
                ...
                <select
                  style={styles.select}
                  value={lieuExcel}
                  onChange={(e) => setLieuExcel(e.target.value)}
                >
            ...
            <div>
              ...
              <div style={styles.row}>
                ...
                <select
                  style={styles.select}
                  value={isNuit ? "Oui" : "Non"}
                  onChange={(e) => setIsNuit(e.target.value === "Oui")}
                >
                  <option>Non</option>
                  <option>Oui</option>
                </select>
              ...
              <div style={styles.row}>
                ...
                <select
                  style={styles.select}
                  value={isWE ? "Oui" : "Non"}
                  onChange={(e) => setIsWE(e.target.value === "Oui")}
                >
                  <option>Non</option>
                  <option>Oui</option>
                </select>
        ...
        <div style={styles.card}>
          ...
          {current ? (
              ...
              <div>
                ...
                {useForfait ? (
                  <>
                    ...
                    <KV
                      k="Détail forfait"
                      v={`${fmt(forfaitKm, 2)} km • ${Math.max(1, forfaitDeliveries)} livraison(s)`}
                    />
                  </>
                ) : (
            ...
            <div style={{ fontSize: 12, color: "var(--muted)" }}>
              Importe un Excel et choisis un lieu pour afficher le calcul.
            </div>
          )}
        </div>
        ...
        <div style={styles.card}>
          ...
          <div ref={invoiceRef} style={styles.invoiceWrap}>
            ...
            <div style={styles.invoiceGrid2}>
              ...
              <div>
                ...
                <div>
                  {useParis
                    ? `Trajet: ${PARIS_ARR.find((a) => a.id === arrDepart)?.label || "—"} → ${
                        PARIS_ARR.find((a) => a.id === arrArrivee)?.label || "—"
                      }`
                    : "Distance manuelle"}
                </div>
                <div>
                  Distance: {fmt(distance, 2)} km • Temps: {minutes} min
                </div>
                <div>
                  Mode déplacement: {useForfait ? "Forfait livraison" : "Tarif Excel (forfait + km)"}
                </div>
              </div>
            </div>
            ...
            <table style={styles.table}>
              ...
              <tbody>
                <tr>
                  <td style={styles.td}>
                    Main d'œuvre (durée facturable {fmt(res?.dureeH ?? 0, 2)} h)
                  </td>
                ...
                <tr>
                  <td style={styles.td}>
                    Déplacement{" "}
                    {useForfait
                      ? `(forfait ${fmt(forfaitKm, 2)} km, ${Math.max(1, forfaitDeliveries)} livr.)`
                      : "(forfait + km Excel)"}
                  </td>
                  <td style={styles.td}>1</td>
                  <td style={styles.td}>—</td>
          ...
          <div style={styles.btnRow}>
            <button
              ...
              onClick={() =>
                invoiceStyle === "transitainer"
                  ? handleExportPDFTransitainer()
                  : handleExportPDF()
              }
            >
              Exporter en PDF
            ...
            {!res && (
              <span style={styles.small}>
                Importe un Excel et calcule d’abord un résultat.
              </span>
            )}
          </div>
        </div>
...
function KV({ k, v, suf, strong, big }) {
  ...
  const styleKey = { color: "var(--muted)" };
  const styleVal = { fontWeight: strong ? 700 : 400 };
  const val =
    typeof v === "number"
